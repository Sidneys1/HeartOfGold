...
namespace HeartOfGold.MonoGame.Helpers {
    public static class DataHelpers {
        public static string[] DataSizes = {"B", "KB", "MB", "GB", "TB"};
        public static string NormalizeFileSize(long bytes) {
            var size = 0;
            double b = bytes;
            while (b >= 1024) {
                b /= 1024;
            ...
            return $"{b:F2}{DataSizes[size]}";
        }
    }

    public static class Easing {
        public static float Ease(double linearStep, float acceleration, EasingType type) {
            var easedStep = acceleration > 0
                ? EaseIn(linearStep, type)
                : acceleration < 0
                    ? EaseOut(linearStep, type)
                    : (float) linearStep;

            return AdvancedMathHelper.Lerp(linearStep, easedStep, Math.Abs(acceleration));
        }
        ...
        public static float EaseInOut(double linearStep, EasingType easeInType, EasingType easeOutType)
            => linearStep < 0.5 ? EaseInOut(linearStep, easeInType) : EaseInOut(linearStep, easeOutType);
        ...
        private static class Sine {
            public static float EaseIn(double s) => (float) Math.Sin(s*AdvancedMathHelper.HALF_PI - AdvancedMathHelper.HALF_PI) + 1;

            public static float EaseOut(double s) => (float) Math.Sin(s*AdvancedMathHelper.HALF_PI);

            public static float EaseInOut(double s) => (float) (Math.Sin(s*AdvancedMathHelper.PI - AdvancedMathHelper.HALF_PI) + 1)/2;
        }

        private static class Power {
            public static float EaseIn(double s, int power) => (float) Math.Pow(s, power);
    ...
    public static class AdvancedMathHelper {
        public const float PI = (float)Math.PI;
        public const float HALF_PI = (float)(Math.PI / 2);

        public static float Lerp(double from, double to, double step) => (float)((to - from) * step + from);
    }
}